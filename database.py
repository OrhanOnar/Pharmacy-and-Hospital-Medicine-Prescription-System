...
def patient_history(patientno):
    with engine.connect() as con:
        history = con.execute('SELECT * FROM PRESCRIPTION WHERE "sno_pat"=:no', no=patientno)
        resultset = []
        for row in history:
            resultset.append(dict(row))
        return resultset
...
def find_name(usersno):
    with engine.connect() as con:
        name = con.execute('SELECT "fname","lname" FROM PERSON WHERE "sno"=:no', no=usersno)
        resultset = []
        for row in name:
            resultset.append(dict(row))
        result = resultset[0]['fname'] + " " + resultset[0]['lname']

    return result


def prescription(pres_no):
    with engine.connect() as con:
        history = con.execute('SELECT * FROM PRESCRIPTION WHERE "pres_no"=:no', no=pres_no)
        resultset = []
        for row in history:
            resultset.append(dict(row))
        return resultset
